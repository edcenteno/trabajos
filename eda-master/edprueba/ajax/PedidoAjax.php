        ...
        $i = 1;
            while ($reg = $query_Pedido->fetch_object()) {
                ...
                $data[] = array("0"=>$i,
                    "1"=>$reg->Cliente,
                    "2"=>$reg->idpedido,
                    "3"=>($reg->tipo_pedido=="Pedido")?'<span class="badge bg-blue">Pedido</span>':(($reg->tipo_pedido=="Venta")?'<span class="badge bg-aqua">Venta</span>':'<span class="badge bg-green">Proforma</span>'),
                    ...
                    "10"=>($reg->estado=="A")?'<span class="badge bg-green">ACEPTADO</span>':'<span class="badge bg-red">CANCELADO</span>',
                    "11"=>'<button class="btn btn-success" data-toggle="tooltip" title="Ver Detalle" onclick="cargarDataPedido('.$reg->idpedido.',\''.$reg->tipo_pedido.'\',\''.$reg->numero.'\',\''.$reg->Cliente.'\',\''.$reg_total->Total.'\',\''.$reg->email.'\')" ><i class="fa fa-eye"></i> </button>&nbsp'.
                    ...
                    '<a href="./Reportes/exVenta.php?id='.$reg->idpedidov.'" class="btn btn-primary" data-toggle="tooltip" title="Imprimir" target="blanck" ><i class="fa fa-file-text"></i> </a>&nbsp;'.
                    '<a href="./Reportes/MVenta.php?id='.$reg->idpedidov.'" class="btn btn-primary" data-toggle="tooltip" title="Imprimir" target="blanck" ><i class="fa fa-file-text"></i> </a>&nbsp;');
        ...
        $i = 1;
            while ($reg = $query_cli->fetch_object()) {
                $data[] = array(
                    ...
                    data-descripcion="'.$reg->descripcion.'" data-nombre="'.$reg->Articulo.'" data-precio-venta="'.$reg->precio_ventapublico.'"
                    ...
                    onclick="AgregarPedCarrito('.$reg->iddetalle_ingreso.',\''.$reg->stock_actual.'\',\''.$reg->Articulo.'\',\''.$reg->codigo.'\',\''.$reg->descripcion.'\',\''.$reg->precio_ventapublico.'\')" >
