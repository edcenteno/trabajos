...
function fact_devM( $libelle, $num )
{
    $r1  = $this->w - 88;
    $r2  = $r1 + 48;
    $y1  = 12;
...
function addColsM( $tab )//tabla encabezado
{
	global $colonnes;
...
function addLineM( $ligne, $tab )
{
	...
	while ( list( $lib, $pos ) = each ($colonnes) )
	{
		...
		$formText  = $format[ $lib ];
		$this->SetXY( $ordonnee, $ligne-1);
		$this->MultiCell( $longCell, 6 , $texte, 0, $formText);
		if ( $maxSize < ($this->GetY()  ) )
			$maxSize = $this->GetY() ;
		$ordonnee += $pos;
	}
...
function addCadreEurosFrancsM($impuesto)
{
	$r1  = $this->w - 50;
	$r2  = $r1 + 90;
	$y1  = $this->h - 145;
...
function addTVAsM( $igv, $total,$moneda )
{
	$this->SetFont('Arial','',7);
