<?php
	...
	class DocSucursal{
		...
		public function ListarDetalleDocSuc($idsucursal){
			global $conexion;
				$sql = "select dds.*, td.nombre ,p.idpedido
		from detalle_documento_sucursal dds inner join tipo_documento td on dds.idtipo_documento = td.idtipo_documento
	    inner join pedido p on dds.idsucursal=p.idsucursal
		where dds.idsucursal = $idsucursal";
		

	/*select dds.*, td.nombre 
	from detalle_documento_sucursal dds inner join tipo_documento td on dds.idtipo_documento = td.idtipo_documento
	where dds.idsucursal = $idsucursal*/

	/*select dds.*, td.nombre ,p.idpedido
	from detalle_documento_sucursal dds inner join tipo_documento td on dds.idtipo_documento = td.idtipo_documento
    join pedido p on dds.idsucursal=pedido.idsucursal
	where dds.idsucursal = 1*/
			$query = $conexion->query($sql);
			return $query;
		}
