<?php
	...
	class Ingreso{
		...
		public function Registrar($idusuario, $idsucursal, $idproveedor, $tipo_comprobante, $serie_comprobante, $num_comprobante, $impuesto, $total, $detalle){
			...
			try {
				...
				$sql = "INSERT INTO ingreso(idusuario, idsucursal, idproveedor, tipo_comprobante, serie_comprobante, num_comprobante, fecha,hora, impuesto,
						total, estado) 
						VALUES($idusuario, $idsucursal, $idproveedor, '$tipo_comprobante', '$serie_comprobante', '$num_comprobante', curdate(),'$hora', $impuesto, $total, 'A')";
				...
				foreach($detalle as $indice => $valor){
					$sql_detalle = "INSERT INTO detalle_ingreso(idingreso, idarticulo, codigo, serie, descripcion, stock_ingreso, stock_actual, precio_compra, precio_ventadistribuidor, precio_ventapublico)
											VALUES($idingreso, ".$valor[0].", '".$valor[1]."', '".$valor[3]."', '".$valor[2]."', ".$valor[4].", ".$valor[4].", ".$valor[6].", ".$valor[7].", ".$valor[8].")";
		...
		public function Listar($idsucursal){
			global $conexion;
			$sql = "select i.idingreso,i.idusuario,i.idsucursal,i.idproveedor,i.tipo_comprobante,i.serie_comprobante,i.num_comprobante,i.fecha,
			i.hora,i.impuesto,i.total,i.estado,p.num_documento,p.direccion_departamento,p.email,
			 p.nombre as proveedor from ingreso i inner join persona p on i.idproveedor = p.idpersona 
	...
	where ped.idingreso = $idingreso";
		...
		public function ListarProveedor(){
			global $conexion;
			$sql = "select * from persona where tipo_persona = 'Cliente' and estado = 'A'";
			$query = $conexion->query($sql);
			return $query;
		}
