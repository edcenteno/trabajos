<?php
	...
	class Pedido{
		...
		public function Registrar($idcliente, $idusuario, $idsucursal, $tipo_pedido, $numero, $detalle){
			...
			$hora = date("H:i:s");
				...
				$sql = "INSERT INTO pedido(idcliente, idusuario, idsucursal, tipo_pedido, fecha,hora,numero, estado)
						VALUES($idcliente, $idusuario, $idsucursal, '$tipo_pedido', curdate(),'$hora', $numero, 'A')";
				//var_dump($sql);
				$conexion->query($sql);
		...
		public function Listar($idsucursal){
			global $conexion;
			/*$sql = "select v.idpedido as idpedidov,p.*, c.nombre as Cliente, c.email , v.serie_comprobante, v.num_comprobante, v.tipo_comprobante
			from venta v INNER join pedido p1 on v.idpedido= p1.idpedido ,pedido p inner join persona c on p.idcliente = c.idpersona where p.idsucursal = $idsucursal
			and c.tipo_persona = 'Cliente' and p.tipo_pedido = 'Venta' order by v.idpedido desc limit 0,2999";*/

			/*select v.idpedido as idpedidov,p1.idpedido,p1.idcliente,p1.idusuario,p1.idsucursal,p1.tipo_pedido,p1.fecha,p1.hora,p1.numero,p1.estado, c.nombre as Cliente, c.email , v.serie_comprobante, v.num_comprobante, v.tipo_comprobante
				from venta v INNER join pedido p1 on v.idpedido= p1.idpedido inner join persona c on p1.idcliente = c.idpersona where p1.idsucursal = $idsucursal
				and c.tipo_persona = 'Cliente' and p1.tipo_pedido = 'Venta' order by v.idpedido desc limit 0,2999*/
				
			$sql = "select v.idpedido as idpedidov,p1.*, c.nombre as Cliente, c.email , v.serie_comprobante, v.num_comprobante, v.tipo_comprobante
			from venta v INNER join pedido p1 on v.idpedido= p1.idpedido inner join persona c on p1.idcliente = c.idpersona where p1.idsucursal = $idsucursal
			and c.tipo_persona = 'Cliente' and p1.tipo_pedido = 'Venta' order by v.idpedido desc limit 0,2999";
	...
	from detalle_pedido where idpedido = $idpedido";
        ...
        public function TotalPedidoImpuesto($idpedido, $IgvPorcentaje){
			global $conexion;
			$sql = "select sum((cantidad * precio_venta) - descuento) as Total , round( (sum((cantidad * precio_venta) - descuento) *$IgvPorcentaje)/(100+$IgvPorcentaje),2) as IgvTotal, round( sum((cantidad * precio_venta) - descuento) - (sum((cantidad * precio_venta) - descuento) *$IgvPorcentaje)/(100+$IgvPorcentaje),2) as SubTotal, descuento as st from detalle_pedido where idpedido = $idpedido";
			$query = $conexion->query($sql);
			return $query;
		}
		...
		public function GetDetallePedido($idpedido){
			global $conexion;
			$sql = "select a.nombre as articulo, dg.codigo, a.descripcion, dg.serie, dp.*
	...
	where dp.idpedido = $idpedido";
		...
		public function ListarDetalleIngresos($idsucursal){
			global $conexion;
			$sql = "select distinct di.iddetalle_ingreso, di.stock_actual, a.nombre as Articulo, di.codigo, a.descripcion, di.precio_ventapublico, a.imagen, i.fecha
	...
	where ped.idpedido = $idpedido";
		...
		public function GetVenta($idpedido){
			global $conexion;
			$sql = "select p.*, ped.fecha, s.razon_social, v.num_comprobante,v.tipo_comprobante, v.serie_comprobante, s.tipo_documento, s.num_documento as num_sucursal, s.direccion, s.telefono as telefono_suc, s.email as email_suc, s.representante, s.logo, ped.tipo_pedido,v.impuesto,p.tipo_documento as doc
	from persona p inner join pedido ped on ped.idcliente = p.idpersona
	inner join sucursal s on ped.idsucursal = s.idsucursal
	...
	where ped.idpedido = $idpedido";
		...
		public function ImprimirDetallePedido($idpedido){
			global $conexion;
			$sql = "select di.codigo,a.descripcion, a.nombre as articulo, dp.*, (dp.cantidad * dp.precio_venta) - dp.descuento as sub_total
